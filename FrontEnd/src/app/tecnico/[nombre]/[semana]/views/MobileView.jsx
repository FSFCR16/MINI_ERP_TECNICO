...
import { BuscadorRegistros } from "../components/BuscadorRegistros"; // ← ajusta el path
...
export function MobileView({
  ...
  return (
    <div 
      ...
      style={{ paddingBottom: "100px" }}  // 👈 cambia este número hasta que funcione
    >
      ...
      <div className="bg-white/60 backdrop-blur-2xl border border-white/50 rounded-3xl shadow-sm px-4 py-3 flex items-center justify-between">
        ...
        <button
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-xl bg-white/50 border border-white/50 text-amber-600 font-medium shadow-sm active:scale-95 transition"
      ...
      <div className="flex flex-col gap-3">
        {listaVisible.length === 0 && listRegistro.length > 0 && (
          <p className="text-center text-xs text-slate-400 py-6">Sin resultados para esa búsqueda</p>
        )}

        {listaVisible.map((row, index) => {
          // índice real en listRegistro para actualizarCeldaRegistro
              ...
              <div className={`transition-all duration-300 ease-in-out overflow-hidden ${abierto ? "max-h-[1200px] opacity-100" : "max-h-0 opacity-0"}`}>
                ...
                {modoEdicion ? (
                  ...
                  <div className="px-4 py-3 grid grid-cols-2 gap-x-4 gap-y-2.5">
                    {columnasTablaGeneral
                      .filter(col => col.key !== "check_box" && col.key !== "valor_servicio" && col.key !== "total")
                      .map(col => (
                ...
                <div className="mx-4 mb-3 mt-1 flex gap-2">
                  ...
                  <div className={`flex-1 border rounded-2xl p-2.5 text-center ${totalNegativo ? "bg-rose-50/60 border-rose-100/60" : "bg-slate-50/60 border-slate-100/60"}`}>
      ...
      {/* ===== FAB ===== */}
      {fabAbierto && <div className="fixed inset-0 z-40" onClick={() => setFabAbierto(false)} />}
      ...
      <div className="fixed bottom-20 right-4 flex flex-col items-end gap-2 z-50">
        ...
        <button
          ...
          className={`w-[52px] h-[52px] rounded-full bg-white/70 backdrop-blur-2xl border border-white/60 text-slate-700 shadow-lg flex items-center justify-center transition-all duration-300 active:scale-95 ${fabAbierto ? "rotate-45" : ""}`}
        >
