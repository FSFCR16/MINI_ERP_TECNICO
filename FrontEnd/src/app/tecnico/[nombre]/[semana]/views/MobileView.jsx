...
export function MobileView({
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-blue-100 via-sky-100 to-indigo-100 p-4 flex flex-col gap-5 pb-28">
      ...
      {listRegistro.length > 0 && (
        <div className="flex justify-between items-center px-1">
          <p className="text-xs font-semibold text-slate-600">
            {listRegistro.length} registro{listRegistro.length !== 1 ? "s" : ""}
          </p>
          {elementosAEliminar.length > 0 && (
            ...
            >
              Eliminar {elementosAEliminar.length} seleccionado{elementosAEliminar.length !== 1 ? "s" : ""}
            </button>
          )}
        </div>
      )}
